...
Многие утилиты (например, `AutomatonVisualizer::ExportToDot`) ожидают существования директории `res` в корневой папке запуска. Убедитесь, что она создана, или проверьте `std::filesystem::current_path()`, чтобы понять, откуда запускается программа.
